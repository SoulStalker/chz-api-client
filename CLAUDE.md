...
### 1. Недоработки и ошибки в реализации

*   **Неверные типы документов для входящих:** В README и маршрутах по-прежнему указан тип `LP_ACCEPT_GOODS` для получения входящих документов. Согласно документации (Приложение 1), `LP_ACCEPT_GOODS` — это документ приёмки, который формирует сам участник. Входящие отгрузки от поставщиков имеют типы `LP_SHIP_GOODS`, `LP_SHIP_GOODS_CSV` или `LP_SHIP_GOODS_XML`.
*   **Устаревшая модель пагинации:** В параметрах маршрута `/docs` до сих пор указан `page_num`. В True API v4 для метода `doc/list` используется **курсорная пагинация**: параметры `did` (ID последнего документа) и `orderedColumnValue` (значение даты последнего документа). Использование номеров страниц может привести к некорректной выдаче данных.
*   **Отсутствие кэширования JWT:** В разделе «Ограничения» указано, что токен не кэшируется. Это критическая недоработка, так как токен True API действителен до 10 часов. Постоянные запросы на авторизацию через gRPC `sign-service` при каждом обновлении страницы создают лишнюю нагрузку и замедляют работу интерфейса.
*   **Логика отображения ОСУ (Объемно-сортового учета):** Если переделка «типов упаковки» коснулась только названий, то программа может по-прежнему пытаться искать «вложения» там, где их нет. Для кодов с `codeType: "OSU"` (маска `02[gtin]37[count]`) система должна отображать GTIN и количество, не предлагая переход вглубь.

### 2. Что сделано неверно в детальном просмотре

Если реализация деталей УПД не позволяет провалиться внутрь транспортной упаковки (КИТУ), она не соответствует ТЗ и макетам:
*   **Метод получения состава:** Для отображения «Транспортной упаковки» (как на «траспортная_упаовка.png») недостаточно данных из `doc/{docId}/info`. Нужно вызывать `POST /api/v3/true-api/cises/info`, передавая код КИТУ, чтобы получить массив `child`.
*   **Иерархия маршрутов:** Судя по списку маршрутов, реализован только `/docs`. Для полноценной работы как в ТЗ, необходимы маршруты вида `/docs/:id/pack/:code`, чтобы обеспечить drill-down навигацию.

### 3. Как нужно переделать (План действий)

1.  **Исправить фильтр типов документов:** В запросе `doc/list` для входящих использовать `input=true` и типы `LP_SHIP_GOODS...`, а для исходящих — `input=false`.
2.  **Реализовать курсорную пагинацию:**
    *   В Go-клиенте добавить поля `Did` и `OrderedColumnValue`.
    *   В UI (Templ) при наличии `nextPage: true` в ответе API передавать значения из последнего элемента списка в ссылку кнопки «Далее».
3.  **Добавить кэширование токена:** Внедрить простую логику в сервис: если `crpt_token` в куках или памяти еще валиден (проверять время истечения), использовать его вместо вызова `sign-service`.
4.  **Разделить логику типов кодов в шаблонах:**
    *   Если `codeType == "OSU"`, выводить только информацию о партии.
    *   Если код является агрегатом (тип `BOX` или `GROUP`) и содержит вложения (параметр `child` в `cisInfo`), делать его ссылкой на `/docs/:id/pack/:code`.
5.  **Добавить хендлер для вложений:** Создать обработчик, который принимает код упаковки, вызывает `cises/info` и возвращает `templ`-компонент со списком вложенных марок (как в «групповая_упаковка.png»).

**Важно:** Убедитесь, что при запросе деталей всегда передается `body=true`, иначе список товаров в ответе будет пустым.
