...
## Заказы WMS (ord_zakaz) — план реализации

Показать WMS-заказы из `ord_zakaz` и дать возможность вручную привязать каждый заказ к CRPT-документу (УПД). После привязки код заказа отображается в таблице документов и попадает в XML-выгрузку.

### Структура таблиц БД

**`ord_zakaz`** — заголовки заказов:

| Колонка | Тип | Описание |
|---|---|---|
| `kod` | VARCHAR(50) PK | Уникальный код заказа (из XML-атрибута `Код`) |
| `data` | DATE | Дата заказа |
| `nomer` | VARCHAR(200) | Составной номер заказа, напр. `-12864594 12873421` |
| `post_kod` | VARCHAR(50) | Код поставщика — мягкая ссылка на `contractors.code` |
| `post_imya` | VARCHAR(500) | Имя поставщика (денормализовано из XML) |
| `post_inn` | VARCHAR(20) | ИНН поставщика — мягкая ссылка на `contractors.inn` |
| `data_postavki` | DATE | Ожидаемая дата поставки |
| `naklad_kod` | VARCHAR(100) | Номер накладной — может совпадать с `orders.order_id` |
| `summa` | NUMERIC(15,4) | Сумма заказа |
| `updated_at` | TIMESTAMP | Время последнего обновления записи |
| `crpt_doc_id` | VARCHAR(100) | Ручная привязка к CRPT-документу (новая колонка) |

**`ord_zakaz_lines`** — строки заказов:

| Колонка | Тип | Описание |
|---|---|---|
| `id` | SERIAL PK | Автоинкремент |
| `zakaz_kod` | VARCHAR(50) | FK → `ord_zakaz.kod` |
| `tovar_kod` | VARCHAR(50) | Код товара — мягкая ссылка на `products.code` |
| `shk` | VARCHAR(20) | Штрих-код GTIN-14 — мягкая ссылка на `products.gtin` |
| `naim` | VARCHAR(500) | Наименование товара (денормализовано из XML) |
| `kolvo` | NUMERIC(15,3) | Количество |
| `summa` | NUMERIC(15,4) | Сумма по строке |

> Связи с `contractors` и `products` **мягкие** (без FK-ограничения в БД).

### Шаги реализации

#### Шаг 1 — Конфиг и подключение к PostgreSQL
- `config/config.go`: добавить `DBConfig { DSN string \`yaml:"dsn" env:"DB_DSN"\` }` в `Config`
- `config/example.yml`: добавить секцию `db: dsn: "postgres://..."`
- `go.mod`: зависимость `github.com/jackc/pgx/v5`
- `cmd/server/main.go`: открыть `pgxpool.New(ctx, cfg.DB.DSN)`, паниковать при ошибке

#### Шаг 2 — Модели
- `internal/model/zakaz.go` (новый): структуры `Zakaz`, `ZakazLine`

#### Шаг 3 — Миграция
- `migrations/001_add_crpt_doc_id.sql`:
  ```sql
  ALTER TABLE ord_zakaz ADD COLUMN IF NOT EXISTS crpt_doc_id VARCHAR(100);
  ```

#### Шаг 4 — Репозиторий
- `internal/repository/zakaz.go` (новый), методы:
  - `List(ctx, dateFrom, dateTo time.Time, offset, limit int) ([]Zakaz, int, error)`
  - `Get(ctx, kod string) (*Zakaz, []ZakazLine, error)`
  - `FindByCRPTDoc(ctx, crptDocID string) (*Zakaz, error)`
  - `FindByDocIDs(ctx, ids []string) (map[string]Zakaz, error)` — для пакетного обогащения списка документов
  - `SetCRPTDocID(ctx, kod, crptDocID string) error`
  - `ClearCRPTDocID(ctx, kod string) error`

#### Шаг 5 — Хендлер заказов
- `internal/web/handlers/zakazy.go` (новый)
- `ZakazHandler` принимает: `*repository.ZakazRepo`, `*crpt.Client`, `*session.Store`

| Метод | Путь | Логика |
|-------|------|--------|
| GET | `/zakazy` | Список с фильтром по дате и OFFSET-пагинацией |
| GET | `/zakazy/:kod` | Детали + строки + кандидаты CRPT для привязки |
| POST | `/zakazy/:kod/match` | Сохранить `crpt_doc_id`, редирект обратно |
| POST | `/zakazy/:kod/unmatch` | Снять привязку |

**Подбор кандидатов** на `/zakazy/:kod`:
1. Взять `post_inn` из заказа
2. `crpt.Client.ListDocuments(dateFrom=data_postavki-7d, dateTo=data_postavki+7d, input=true)`
3. Отфильтровать по `senderInn == post_inn`
4. Показать список — пользователь нажимает «Привязать»

#### Шаг 6 — Изменения в DocsHandler
- `DocsHandler` получает поле `zakazy *repository.ZakazRepo`
- `ListDocuments`: после получения списка CRPT-документов вызвать `repo.FindByDocIDs(docNumbers)` → передать карту `map[docID]Zakaz` в шаблон
- `ExportDocumentXML`: вызвать `repo.FindByCRPTDoc(docID)` → заполнить `ZakazKod` в XML

#### Шаг 7 — Изменения в XML-модели
- `internal/model/export.go`: добавить `ZakazKod string \`xml:"КодЗаказа,attr,omitempty"\`` в `ExportDocument`

#### Шаг 8 — Шаблоны
- `views/zakazy.templ` (новый): фильтр дат + таблица заказов + пагинация
- `views/zakaz_detail.templ` (новый): реквизиты + блок привязки (кандидаты / текущая привязка) + таблица строк
- `views/documents.templ`: добавить колонку «Код заказа WMS»
- `views/layout.templ`: пункт «Заказы» в навигацию

#### Шаг 9 — Роутинг
- `cmd/server/main.go`: создать репозиторий, передать в оба хендлера, зарегистрировать маршруты `/zakazy`
